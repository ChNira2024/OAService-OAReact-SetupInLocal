...
public class ReserveFundDetailsServiceImpl implements ReserveFundDetailsService {
	...
	@Value("${GET_ACCOUNT_TYPE_URL}")
	private String getAccountClsType;
	...
	public void updateReserveFundData(Integer mgmtCompId, Integer propId, String periodCode) {
		...
		Integer buildingId = reserveFundDao.getBuildingId(propId);  //here i got buildingId by passing propId
		LOGGER.info("Building Id is::" + buildingId);
		Integer reserveFundId = reserveFundDao.getReserveFundId(mgmtCompId, buildingId); //here i got reserveFundId by passing mgmtCompId,buildingId
		LOGGER.info("Reserver_Fund Id is::" + reserveFundId);

		if (periodCode == null || periodCode == "") //from postman if ur not passing periodCode then it will take current year
		{
		...
		Double reserveFund = reserveFundDao.getReserveFund(mgmtCompId, propId, periodCode); //here i got Total_Budget value for reserveFund 

		Double generalFund = reserveFundDao.getGeneralFund(mgmtCompId, propId, periodCode); //here i got Total_Budget value for generalFund
		...
		Double percentage = (reserveFund) / (generalFund + reserveFund) * 100; // raghuram  //calculate the percentage
		...
		details = reserveFundDao.getAccountDetails(mgmtCompId, buildingId);  //here i got all OA_RESERVEFUND_DETAILS data by passing mgmtCompId,buildingId
		...
		int insertresponse = reserveFundDao.UpdateStatus(mgmtCompId, convertedValue, buildingId);//here i have updated "percentage" and "lastCalculatedOn" in OA_RESERVE_FUND by using mgmtCompId ang buildingId
	...
	//Account cls type
	public String getAccountClsType(String accNumber)
	{
		...
		LOGGER.info("GET AACOUNT TYPE URL:"+getAccountClsType+accNumber);
		
		HttpEntity<String> request = new HttpEntity<String>(headers);
		try {
			apiResponse = restTemplate.exchange(getAccountClsType+accNumber,HttpMethod.GET,request,String.class);
			if(apiResponse.getStatusCode().is2xxSuccessful())
			{
				...
				String accountClsType = object.get("accCls").getAsString();
				return accountClsType;
			}
			else
			{
				LOGGER.info("Error:"+apiResponse.getStatusCode());
				return "Unable to get account class type:"+apiResponse.getStatusCode();
			}
		}
		catch(Exception e)
		{
			LOGGER.error("Error occur while making api call:"+e.getMessage());
			e.printStackTrace();
			return "Error occur while making api call:"+e.getMessage();
		}
