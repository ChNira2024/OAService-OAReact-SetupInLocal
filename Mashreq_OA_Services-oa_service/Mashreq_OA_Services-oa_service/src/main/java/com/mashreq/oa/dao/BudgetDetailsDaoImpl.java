...
//			logger.info("Balance Amount:" + previousConsumedAmount);
	...
	@Override
	public List<BudgetDetailsOutput> getBudgetItemById(Integer budgetItemId) 
	{
	...
	public void logAuditTrail(BudgetDetailsOutput budgetItemsData, AuditTrail2 auditTrail,Double amount,String username, String comment) 
	{
        ...
        String sql = "INSERT INTO OA_AUDIT_TRAIL_LOG (BUDGET_ITEM_ID, PYMT_REQ_ID, SERVICE_CODE, REVERSAL_AMOUNT, UPDATEDBY, UPDATEDON, \"COMMENT\")VALUES (?, ?, ?, ?, ?, ?, ?)";
	...
	@Override
	public List<AuditTrailLog> fetchAuditTrailLog(String serviceCode, String userName, Date updatedFrom,Date updatedTo,String mgmtCompId,String bildingId) 
	{
		...
		*/
		...
		/*String sql = "SELECT BUDGET_ITEM_ID as budgetItemId,PYMT_REQ_ID,SERVICE_CODE,REVERSAL_AMOUNT,UPDATEDBY,UPDATEDON,\"COMMENT\",MGMT_COMP_ID,BUILDING_ID FROM OA_AUDIT_TRAIL_LOG "
				+ "WHERE SERVICE_CODE = ? AND UPDATEDBY = ? AND UPDATEDON >= TO_DATE(?,'dd-MM-yy') AND UPDATEDON < TO_DATE(?,'dd-MM-yy')";*/
		
		String sql = "SELECT BUDGET_ITEM_ID as budgetItemId,PYMT_REQ_ID,SERVICE_CODE,REVERSAL_AMOUNT,UPDATEDBY,TO_CHAR(UPDATEDON, 'YYYY-MM-DD HH24:MI:SS') as UPDATEDON,\"COMMENT\",MGMT_COMP_ID,BUILDING_ID FROM OA_AUDIT_TRAIL_LOG "
				+ "WHERE MGMT_COMP_ID = ? AND BUILDING_ID = ? AND SERVICE_CODE = ? AND UPDATEDBY = ? AND UPDATEDON >= TO_DATE(?,'dd-MM-yy') AND UPDATEDON < TO_DATE(?,'dd-MM-yy')";
		...
		try {
			fetchedListData = jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(AuditTrailLog.class),new Object[] {mgmtCompId,bildingId,serviceCode, userName, newUpdatedFrom, newUpdatedTo });
	...
	public void updateBudgetItemTableWithServiceCode(BudgetDetailsOutput budgetItem) {
		logger.info("inside com.mashreq.oa.dao:BudgetDetailsDaoImpl class having updateBudgetItemTableWithServiceCode() method");
